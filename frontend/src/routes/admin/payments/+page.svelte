<script lang="ts">
	...
	import type { Payment, AdminUser } from '$lib/types';
	...
	async function loadPayments() {
		...
		errorMessage = '';
		try {
			const data = await apiCallWithAuth<Payment[]>('/payments');
			payments = data || [];
		} catch (err) {
			errorMessage = err instanceof Error ? err.message : '支払い記録の取得に失敗しました';
		} finally {
			isLoading = false;
		}
	}
	...
	function openEditModal(payment: Payment) {
		...
		modalDate = new Date(payment.paid_at).toISOString().split('T')[0];
	...
	async function handleModalSubmit() {
		...
		try {
			const paidAt = new Date(modalDate).toISOString();
			if (modalMode === 'add') {
				await apiCallWithAuth('/payments', {
					method: 'POST',
					body: JSON.stringify({
						user_id: modalUserId,
				...
				// edit
				await apiCallWithAuth(`/payments/${editingPaymentId}`, {
					method: 'PUT',
					body: JSON.stringify({
						amount: parseInt(modalAmount),
						paid_at: paidAt,
	...
	function openDeleteConfirm(payment: Payment) {
	...
	async function handleDelete() {
		...
		try {
			await apiCallWithAuth(`/payments/${deleteTargetId}`, {
				method: 'DELETE'
			});
			closeDeleteConfirm();
			await loadPayments();
		} catch (err) {
			errorMessage = err instanceof Error ? err.message : '削除に失敗しました';
		} finally {
			isLoading = false;
		}
	}
...
<div class="payments-page">
	...
	{:else if payments.length === 0}
		<div class="empty-notice">支払い記録がありません</div>
	{:else}
...
{#if showModal}
	...
	<div class="modal">
		<div class="modal-header">
			<h2>{modalMode === 'add' ? '支払い記録を追加' : '支払い記録を編集'}</h2>
			<button class="btn-close" onclick={closeModal}>✕</button>
		</div>

		<form onsubmit={(e) => { e.preventDefault(); handleModalSubmit(); }}>
			<div class="form-group">
				<label for="payment-user">ユーザー</label>
				<select
					id="payment-user"
...
{#if showDeleteConfirm}
	...
	<div class="modal modal-small">
		...
		<div class="modal-body">
			<p>
				この支払い記録（<strong>{formatCurrency(deleteTargetAmount)}</strong>）を削除してもよろしいですか？
			</p>
			<p class="notice">この操作は取り消せません。</p>
		</div>

		<div class="modal-footer">
			<button class="btn btn-secondary" onclick={closeDeleteConfirm} disabled={isLoading}>
				キャンセル
			</button>
