<script lang="ts">
	...
	async function loadUsers() {
		...
		try {
			...
			errorMessage = err instanceof Error ? err.message : '利用者一覧の取得に失敗しました';
		} finally {
			isLoading = false;
		}
	}
		...
		try {
			if (modalMode === 'add') {
				...
				await apiCallWithAuth('/users', {
					method: 'POST',
					body: JSON.stringify({ name: modalName, barcode: modalBarcode })
				});
			} else {
				// edit
				...
				<tbody>
					{#each users as user (user.id)}
						<tr class:inactive={user.is_active === 0}>
							...
							<td>
								<span class="status-badge" class:active={user.is_active === 1}>
