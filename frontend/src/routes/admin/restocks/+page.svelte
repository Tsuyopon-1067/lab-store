<script lang="ts">
	...
	async function loadRestocks() {
		...
		try {
			...
			errorMessage = err instanceof Error ? err.message : '仕入れ履歴の取得に失敗しました';
		} finally {
			isLoading = false;
		}
	}
				...
				<tbody>
					{#each restocks as restock (restock.id)}
						<tr class:deleted={isDeleted(restock)}>
							...
							<td class="amount-cell">{formatCurrency(restock.total_amount)}</td>
							<td>{formatDate(restock.restocked_at)}</td>
							<td class="note-cell">{restock.note || '—'}</td>
							<td>
								{#if isDeleted(restock)}
									<span class="status-badge status-deleted">削除済み</span>
								{:else}
							...
							<td class="action-cell">
								{#if !isDeleted(restock)}
									<button
										...
										onclick={() => openDeleteConfirm(restock)}
										disabled={isLoading}
									>
										削除
									</button>
...
{#if showDeleteConfirm}
	...
	<div class="modal modal-small">
		...
		<div class="modal-body">
			<p>
				この仕入れ記録（<strong>{formatCurrency(deleteTargetAmount)}</strong>）を削除してもよろしいですか？
			</p>
