<script lang="ts">
	...
	async function loadRestockPayments() {
		...
		errorMessage = '';
		try {
			const data = await apiCallWithAuth<RestockPayment[]>('/restock-payments');
			restockPayments = data || [];
		} catch (err) {
			errorMessage = err instanceof Error ? err.message : '立替精算記録の取得に失敗しました';
		} finally {
			isLoading = false;
		}
	}
...
{#if showModal}
	...
	<div class="modal">
		<div class="modal-header">
			<h2>{modalMode === 'add' ? '立替精算記録を追加' : '立替精算記録を編集'}</h2>
...
{#if showDeleteConfirm}
	...
	<div class="modal modal-small">
		...
		<div class="modal-body">
			<p>
				この立替精算記録（<strong>{formatCurrency(deleteTargetAmount)}</strong>）を削除してもよろしいですか？
			</p>
